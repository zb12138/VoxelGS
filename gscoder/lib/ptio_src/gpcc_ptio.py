...
class PCC_IO:
    def __init__(self):
        self.lib = ctypes.CDLL(os.path.join(os.path.dirname(__file__), 'ptio_backend.so'))
...
def pcwrite(file_path: str,
    ...
    :param colors: Optional NumPy array of colors (shape: [N, 3]). uint8 
    :param reflectance: Optional NumPy array of reflectance values (shape: [N, 1]). uint16
...
def pcread(file_path: str, 
    ...
    :param attribute: Whether to read color [uint8]/reflectance.[uint16]!
