...
def image_resize(pil_image, resolution):
    orig_w, orig_h = pil_image.size
    if resolution in [1, 2, 4, 8]:  # downsampling times
        resolution = (round(orig_w / resolution), round(orig_h / resolution))
    else:
        if resolution == -1:
            if orig_w > 1600:
...
# def image_resize(pil_image, resolution):
    ...
    # target_shape = resolve_size(pil_image.size)
    # transform = T.Compose([T.Resize(target_shape[::-1]), T.ToTensor()])
